...
class DeviceAiProtocolRunnerTest {
  ...
  fun run_persistsDeterministicReportAndArtifactMetadata() {
    ...
    val runner =
      DeviceAiProtocolRunner(
        ...
        downloadModel = { _, destination, _, _ ->
  ...
  @Test
  fun run_persistsInFlightProgressSnapshotBeforeDownloadCompletes() {
    val rootDirectory = Files.createTempDirectory("device-ai-runner-progress-").toFile()
    val artifactBytes = "native-device-ai-progress".encodeToByteArray()
    val expectedSha256 = sha256Hex(artifactBytes)
    val latestReportFile = File(rootDirectory, "bao-edge-device-ai/protocol/latest.json")
    val runner =
      DeviceAiProtocolRunner(
        appStorageRoot = rootDirectory,
        downloadModel = { _, destination, _, _ ->
          assertTrue(latestReportFile.exists())
          val progressReport = latestReportFile.readText()
          assertTrue(progressReport.contains("\"state\": \"running\""))
          assertTrue(progressReport.contains("\"code\": \"DOWNLOAD_IN_PROGRESS\""))
          destination.parentFile?.mkdirs()
          destination.writeBytes(artifactBytes)
          ModelDownloadResult.Success(file = destination, correlationId = "hf-correlation")
        },
    ...
    val result =
      kotlinx.coroutines.runBlocking {
        runner.run(
          request =
            DeviceAiProtocolRunRequest(
              correlationId = "android-device-ai-progress",
              modelRef = "huggingface.co/mradermacher/AutoGLM-Phone-9B-Multilingual-GGUF",
              revision = "rev-123",
              fileName = "model.gguf",
              expectedSha256 = expectedSha256,
