...
data class Model(
  ...
  /** Optional localized display name resource for UI surfaces. */
  ...
  /** Optional localized model info resource for UI surfaces. */
  @StringRes val infoResId: Int? = null,
  ...
  fun getDisplayName(context: Context): String {
    val resourceValue = displayNameResId?.let(context::getString)?.trim().orEmpty()
  ...
  fun getInfo(context: Context): String {
    val resourceValue = infoResId?.let(context::getString)?.trim().orEmpty()
    if (resourceValue.isNotEmpty()) {
      return resourceValue
    }
    return info
