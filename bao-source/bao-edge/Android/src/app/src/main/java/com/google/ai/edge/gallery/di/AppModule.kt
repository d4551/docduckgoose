...
import com.google.ai.edge.gallery.data.AndroidCredentialStore
import com.google.ai.edge.gallery.data.PreferencesStore
import com.google.ai.edge.gallery.data.SharedPreferencesStore
...
internal object AppModule {
  ...
  fun provideHuggingFaceModelManager(): HuggingFaceModelManager {
    return HuggingFaceModelManager(
      config = HuggingFaceDownloadConfig(maxAttempts = BaoEdgeRuntimeConfig.deviceAiDownloadMaxAttempts)
    )
  }
  ...
  fun provideDeviceAiProtocolRunner(
    ...
    return DeviceAiProtocolRunner(
      ...
      downloadModel = { model, destination, token, onProgress ->
