...
data class DeviceAiProtocolRunRequest(
  ...
  val timeoutMs: Long = BaoEdgeRuntimeConfig.deviceAiProtocolTimeoutMs.toLong(),
)
...
class DeviceAiProtocolRunner(
  ...
  private suspend fun executeProtocol(
    ...
    persistInFlightReport(
      ...
      message = "Downloading required model into app-managed storage.",
      stages = listOf(
        DeviceAiProtocolStageReport(
          name = DEVICE_AI_PROTOCOL_STAGE_VALIDATE,
          status = DeviceAiProtocolStageStatus.PASS,
    ...
    var lastProgressPersistedAtMs = 0L
    val downloadResult =
      downloadModel(
        resolvedModel,
        artifactFile,
        ...
        { progress: ModelDownloadProgress ->
          ...
          val shouldPersist =
            lastProgressPersistedAtMs == 0L ||
              now - lastProgressPersistedAtMs >= 2_000L ||
              (progress.totalBytes > 0L && progress.receivedBytes >= progress.totalBytes)
          if (!shouldPersist) {
            return@downloadModel Unit
          }
          lastProgressPersistedAtMs = now
          val message =
            if (progress.totalBytes > 0L) {
              "Downloading required model (${progress.receivedBytes}/${progress.totalBytes} bytes)."
            } else {
              "Downloading required model (${progress.receivedBytes} bytes received)."
            }
          persistInFlightReport(
            request = request,
            reportFile = reportFile,
  ...
  private fun persistInFlightReport(
    request: DeviceAiProtocolRunRequest,
    reportFile: File,
    latestReportFile: File,
    startedAtEpochMs: Long,
    ...
    runCatching {
      ...
      ensureParentDirectory(latestReportFile)
      val json = gson.toJson(report)
      reportFile.writeText(json)
      latestReportFile.writeText(json)
    }.onFailure { error ->
      logger.error(
        "device_ai_report_progress_write_failed",
        error,
        listOf("reportPath" to reportFile.absolutePath),
      )
    }
  }
  ...
  private fun resolveReportDirectory(): File {
    val baseDirectorySegments =
      BaoEdgeRuntimeConfig.deviceAiManagedModelDirectory
        .split('/')
        .filter { it.isNotBlank() }
        .dropLast(1)
        .ifEmpty { listOf("bao-edge-device-ai") }
