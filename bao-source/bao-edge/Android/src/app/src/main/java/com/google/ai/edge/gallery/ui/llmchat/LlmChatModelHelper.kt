...
object LlmChatModelHelper {
  ...
  fun initialize(
    ...
    StructuredLog.d(
      TAG,
      "llm_chat_engine_initialization_started",
      "model" to model.name,
      "supportsImage" to shouldEnableImage,
      "supportsAudio" to shouldEnableAudio,
      "accelerator" to accelerator,
      "preferredBackend" to preferredBackend,
    )
    ...
    try {
      ...
      StructuredLog.e(TAG, "llm_chat_engine_initialization_failed", e, "model" to model.name)
  ...
  fun resetConversation(
    ...
    try {
      StructuredLog.d(
        TAG,
        "llm_chat_conversation_reset_started",
        "model" to model.name,
      )
      ...
      StructuredLog.d(
        TAG,
        "llm_chat_conversation_reset_configured",
        "model" to model.name,
        "supportsImage" to shouldEnableImage,
        "supportsAudio" to shouldEnableAudio,
      )
      ...
      StructuredLog.d(TAG, "llm_chat_conversation_reset_completed", "model" to model.name)
    } catch (e: Exception) {
      StructuredLog.w(TAG, "llm_chat_conversation_reset_failed", "model" to model.name)
    }
  }
  ...
  fun cleanUp(model: Model, onDone: () -> Unit) {
    ...
    try {
      ...
      StructuredLog.e(TAG, "llm_chat_conversation_close_failed", e, "model" to model.name)
    }

    try {
      instance.engine.close()
    } catch (e: Exception) {
      StructuredLog.e(TAG, "llm_chat_engine_close_failed", e, "model" to model.name)
    }
    ...
    onDone()
    StructuredLog.d(TAG, "llm_chat_cleanup_completed", "model" to model.name)
  }
      ...
      object : MessageCallback {
        ...
        override fun onError(throwable: Throwable) {
          if (throwable is CancellationException) {
            StructuredLog.d(TAG, "llm_chat_inference_cancelled", "model" to model.name)
            resultListener("", true)
          } else {
            StructuredLog.e(TAG, "llm_chat_inference_callback_failed", throwable, "model" to model.name)
