...
fun GlobalModelManager(
      ...
      if (result.resultCode == android.app.Activity.RESULT_OK) {
        result.data?.data?.let { uri ->
          ...
          StructuredLog.d(
            TAG,
            "model_file_selected",
            "has_file_name" to (fileName != null),
            "file_extension" to fileName?.substringAfterLast('.', missingDelimiterValue = ""),
          )
          ...
          StructuredLog.d(TAG, "model_file_selection_empty_result")
        }
      } else {
        StructuredLog.d(TAG, "model_file_selection_cancelled")
      }
    }
  ...
  LaunchedEffect(uiState.modelImportingUpdateTrigger) {
    ...
    val sortedModels = allModelsSet.toList().sortedBy { it.getDisplayName(context = context) }
    builtInModels.clear()
