...
class LlmSingleTurnViewModel @Inject constructor() : ViewModel() {
    ...
    viewModelScope.launch(Dispatchers.Default) {
      ...
      while (model.instance == null) {
        if (System.currentTimeMillis() > initDeadline) {
          StructuredLog.e(
            TAG,
            "single_turn_model_initialization_timed_out",
            "model" to model.name,
            "timeoutMs" to MODEL_INIT_TIMEOUT_MS,
          )
          setPreparing(false)
          setInProgress(false)
          return@launch
  ...
  fun selectPromptTemplate(model: Model, promptTemplateType: PromptTemplateType) {
    StructuredLog.d(
      TAG,
      "single_turn_prompt_template_selected",
      "model" to model.name,
      "templateId" to promptTemplateType.id,
    )
  ...
  fun updateResponse(model: Model, promptTemplateType: PromptTemplateType, response: String) {
    _uiState.update { currentState ->
      ...
      modelResponses[promptTemplateType.id] = response
  ...
  fun stopResponse(model: Model) {
    StructuredLog.d(TAG, "single_turn_stop_requested", "model" to model.name)
    viewModelScope.launch(Dispatchers.Default) {
      setInProgress(false)
