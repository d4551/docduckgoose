...
import com.baohaus.baoedge.rpa.android.AndroidUiAutomatorDriver
...
/** Evaluates flow risk according to Bao Edge tiered guard defaults. */
object BaoEdgeFlowSafetyPolicy {
...
@Singleton
class BaoEdgeFlowConsentRegistry @Inject constructor() {
...
@Singleton
class BaoEdgeRpaFallbackExecutor
@Inject
constructor(
  @ApplicationContext private val context: Context,
  private val consentRegistry: BaoEdgeFlowConsentRegistry,
) {
  ...
  suspend fun execute(
    ...
    val safetyDecision = BaoEdgeFlowSafetyPolicy.evaluate(parsedFlow)
  ...
  private suspend fun runFlow(flow: FlowV1, correlationId: String): JSONObject {
    ...
    return when (result.state) {
      FlowExecutionState.SUCCESS -> {
        ...
        result.payload?.artifacts?.clipboardContent?.let { content ->
          envelope.put("clipboardContent", content)
        }
        envelope
      }
