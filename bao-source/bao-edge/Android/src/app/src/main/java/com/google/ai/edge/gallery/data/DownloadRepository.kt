...
class DefaultDownloadRepository(
      ...
      if (workInfo != null) {
        when (workInfo.state) {
          ...
          WorkInfo.State.CANCELLED -> {
            ...
            StructuredLog.w(
              TAG,
              "download_worker_finished_with_error",
              "worker_id" to workerId.toString(),
              "model_name" to model.name,
              "state" to workInfo.state.name,
              "has_error_message" to rawErrorMessage.isNotEmpty(),
            )
  ...
  private fun categorizeDownloadError(rawMessage: String): String {
    return when {
      ...
      rawMessage.contains("SocketTimeoutException") || rawMessage.contains("timeout", ignoreCase = true) ->
        context.getString(R.string.download_error_timeout)
      ...
      rawMessage.contains("403") ->
        context.getString(R.string.download_error_access_denied)
      rawMessage.contains("404") ->
        context.getString(R.string.download_error_not_found)
  ...
  private fun sendNotification(title: String, text: String, taskId: String, modelName: String) {
    ...
    val channelName = "${BuildConfig.BAO_EDGE_APP_NAME} download notification"
    ...
    // Download from global model manager. Open the global model manager screen.
      ...
      intent =
        Intent(
            Intent.ACTION_VIEW,
            "${BuildConfig.BAO_EDGE_DEEP_LINK_SCHEME}://global_model_manager".toUri(),
          )
      ...
      intent =
        Intent(
            Intent.ACTION_VIEW,
            "${BuildConfig.BAO_EDGE_DEEP_LINK_SCHEME}://model/$taskId/${modelName}".toUri(),
          )
          .apply { flags = Intent.FLAG_ACTIVITY_NEW_TASK }
    }
