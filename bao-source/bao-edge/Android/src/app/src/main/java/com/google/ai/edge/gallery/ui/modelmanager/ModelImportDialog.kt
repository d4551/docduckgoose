...
private val IMPORT_CONFIGS_LLM: List<Config> =
  listOf(
    ...
    SegmentedButtonConfig(
      ...
      allowMultiple = true,
      optionLabelResIds = SUPPORTED_ACCELERATORS.map { it.labelResId },
    ),
  )
...
private fun importModel(
  ...
  coroutineScope.launch(Dispatchers.IO) {
    try {
        ...
        FileOutputStream(outputFile).use { output ->
          ...
          while (input.read(buffer).also { bytesRead = it } != -1) {
            ...
            if (curTs - lastSetProgressTs > 200) {
              StructuredLog.d(
                TAG,
                "model_import_progress_updated",
                "fileName" to fileName,
                "importedBytes" to importedBytes,
                "fileSize" to fileSize,
              )
      ...
      StructuredLog.d(TAG, "model_import_completed", "fileName" to fileName)
      onProgress(1f)
      onDone()
    } catch (e: Exception) {
      StructuredLog.e(TAG, "model_import_failed", e, "fileName" to fileName)
...
private fun getFileSizeAndDisplayNameFromUri(context: Context, uri: Uri): Pair<Long, String> {
  ...
  try {
    ...
    StructuredLog.e(TAG, "model_import_metadata_failed", e)
    return Pair(0L, "")
  }
