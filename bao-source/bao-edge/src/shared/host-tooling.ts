...
import type { Result } from "@baohaus/bao-utils/data-result";

const ANDROID_GRADLE_JVM_BREW_FORMULA = "openjdk@17";
/** Current Android Gradle runtime major selected for the repo build stack. */
export const ANDROID_GRADLE_JVM_MAJOR = 17;
...
const ANDROID_SDK_LICENSE_ACCEPTANCE_REPETITIONS = 256;
const ANDROID_SDK_LICENSE_ACCEPTANCE_INPUT = `${"y\n".repeat(ANDROID_SDK_LICENSE_ACCEPTANCE_REPETITIONS)}`;
const DOCTOR_STATUS_NAME_WIDTH = 28;
const WINDOWS_DEFAULT_PATHEXT = ".EXE;.CMD;.BAT;.COM";
const JAVA_HOME_RESOLVER_PATH = "/usr/libexec/java_home";
...
/** Provenance of the resolved Android Gradle JVM home directory. */
export type AndroidGradleJvmSource = "env" | "macos-java-home" | "brew";

/** Successful Android Gradle JVM resolution details. */
export interface ResolvedAndroidGradleJvm {
  /** Absolute Java home path containing the required Android Gradle runtime. */
  readonly javaHome: string;
  /** Source used to discover the Android Gradle runtime. */
  readonly source: AndroidGradleJvmSource;
  /** Resolved Java major version. */
...
/** Resolve the active Bun-native host environment snapshot used by shared tooling helpers. */
export function getHostToolingEnvironment(): HostToolingEnvironment {
  return { ...Bun.env };
}
...
function buildProcessEnv(env: HostToolingEnvironment): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(getHostToolingEnvironment())) {
...
function resolveCommandPath(
  command: string,
  env: HostToolingEnvironment = getHostToolingEnvironment(),
): string | null {
  const currentEnvironment = getHostToolingEnvironment();
  if (env.PATH === undefined || env.PATH === currentEnvironment.PATH) {
    return Bun.which(command) ?? null;
  }
  const extensions =
    process.platform === "win32"
      ? (env.PATHEXT ?? currentEnvironment.PATHEXT ?? WINDOWS_DEFAULT_PATHEXT)
          .split(";")
          .map((extension) => extension.trim())
          .filter((extension) => extension.length > 0)
      : [""];
  const segments = env.PATH.split(delimiter)
    .map((segment) => segment.trim())
    .filter((segment) => segment.length > 0);
...
function findFirstDirectoryWithSuffix(
  rootPath: string,
  suffix: string,
...
function resolveBrewJavaHome(prefix: string): string | null {
  if (isExecutableFile(join(prefix, "bin", "java"))) {
    return prefix;
  }
  const macBundleHome = join(prefix, "libexec", "openjdk.jdk", "Contents", "Home");
  if (isExecutableFile(join(macBundleHome, "bin", "java"))) {
    return macBundleHome;
  }
  return null;
}

async function resolveMatchingJavaHome(
  javaHome: string,
  major: number,
  source: AndroidGradleJvmSource,
): Promise<{ readonly javaHome: string; readonly source: AndroidGradleJvmSource } | null> {
  const resolvedMajor = await resolveJavaMajorFromHome(javaHome);
  if (resolvedMajor !== major) {
    return null;
  }
...
function resolveConfiguredJavaHomeForMajor(
  ...
  return resolveMatchingJavaHome(configuredJavaHome, major, "env");
}

async function resolveMacosJavaHomeForMajor(
  major: number,
): Promise<{ readonly javaHome: string; readonly source: AndroidGradleJvmSource } | null> {
  if (!(process.platform === "darwin" && isExecutableFile(JAVA_HOME_RESOLVER_PATH))) {
    return null;
  }

  const systemResult = await runHostCommand([JAVA_HOME_RESOLVER_PATH, "-v", String(major)]);
  ...
  return resolveMatchingJavaHome(systemHome, major, "macos-java-home");
}

async function resolveBrewJavaHomeForMajor(
  env: HostToolingEnvironment,
  major: number,
  brewFormula: string,
): Promise<{ readonly javaHome: string; readonly source: AndroidGradleJvmSource } | null> {
  ...
  const brewResult = await runHostCommand(["brew", "--prefix", brewFormula], {
  ...
  const resolvedHome = resolveBrewJavaHome(brewPrefix);
  if (!resolvedHome) {
    return null;
  }

  return resolveMatchingJavaHome(resolvedHome, major, "brew");
}

async function resolveJavaHomeForMajor(
  env: HostToolingEnvironment,
  major: number,
  brewFormula: string,
): Promise<{ readonly javaHome: string; readonly source: AndroidGradleJvmSource } | null> {
  return (
    (await resolveConfiguredJavaHomeForMajor(env, major)) ??
    (await resolveMacosJavaHomeForMajor(major)) ??
    (await resolveBrewJavaHomeForMajor(env, major, brewFormula))
  );
}

/** Resolve the Android Gradle runtime JVM from the active environment, macOS `java_home`, or Homebrew. */
export async function resolveAndroidGradleJvmHome(
  env: HostToolingEnvironment = getHostToolingEnvironment(),
): Promise<ResolvedAndroidGradleJvm | null> {
  const resolved = await resolveJavaHomeForMajor(
    env,
    ANDROID_GRADLE_JVM_MAJOR,
    ANDROID_GRADLE_JVM_BREW_FORMULA,
  );
  if (!resolved) {
    return null;
  }
  return {
    ...resolved,
    major: ANDROID_GRADLE_JVM_MAJOR,
  };
}

/** Build an environment with the Android Gradle runtime JVM activated on `JAVA_HOME` and `PATH`. */
export function createAndroidGradleJvmEnvironment(
  env: HostToolingEnvironment,
  javaHome: string,
...
export function resolveSdkManagerPath(env: HostToolingEnvironment, sdkRoot: string): string | null {
  const candidates = [
...
export function androidSdkPackagesInstalled(sdkRoot: string): boolean {
  return (
    existsSync(join(sdkRoot, "platform-tools")) &&
...
export function resolveDeveloperDirForSimctl(
  ...
  const candidates = [env.DEVELOPER_DIR?.trim() || "", DEFAULT_XCODE_DEVELOPER_DIR];
  for (const candidate of candidates) {
...
/** Ensure the required Android Gradle runtime JVM is available, installing the Homebrew formula when necessary. */
export async function ensureAndroidGradleJvmAvailable(
  env: HostToolingEnvironment = getHostToolingEnvironment(),
): Promise<HostProvisionResult<ResolvedAndroidGradleJvm>> {
  const resolvedHome = await resolveAndroidGradleJvmHome(env);
  if (resolvedHome) {
    return {
      ok: true,
      data: resolvedHome,
  ...
  if (!resolveCommandPath("brew", env)) {
    return {
      ok: false,
      message: `Android builds require Java ${ANDROID_GRADLE_JVM_MAJOR} and Homebrew is unavailable for installation.`,
      stdout: "",
      stderr: "",
    };
  }

  const installResult = await runHostCommand(["brew", "install", ANDROID_GRADLE_JVM_BREW_FORMULA], {
    ...(env.PATH ? { env: { PATH: env.PATH } } : {}),
  });
  const installedHome = await resolveAndroidGradleJvmHome(env);
  if (!(installResult.success && installedHome)) {
    return {
      ok: false,
      message: `Java ${ANDROID_GRADLE_JVM_MAJOR} could not be provisioned for Android Gradle on this host.`,
...
export async function ensureAndroidSdkAvailable(
  ...
  if (!resolvedSdkRoot) {
    ...
    const installSdkRootResult = await runHostCommand(
...
function appendUniqueCandidate(candidates: string[], seen: Set<string>, candidate: string): void {
...
async function readSelectedXcodeDeveloperDirectory(
  env: HostToolingEnvironment,
): Promise<string | null> {
...
function listApplicationXcodeDeveloperDirectories(): string[] {
  if (!existsSync(APPLICATIONS_DIRECTORY)) {
    return [];
  }

  const candidates: string[] = [];
  for (const entry of readdirSync(APPLICATIONS_DIRECTORY, { withFileTypes: true })) {
    if (!(entry.isDirectory() && entry.name.startsWith("Xcode") && entry.name.endsWith(".app"))) {
      continue;
    }

    candidates.push(join(APPLICATIONS_DIRECTORY, entry.name, "Contents", "Developer"));
  }
  return candidates;
}

async function listXcodeDeveloperDirectoryCandidates(
  env: HostToolingEnvironment,
): Promise<string[]> {
  const candidates: string[] = [];
  const seen = new Set<string>();

  appendUniqueCandidate(candidates, seen, env.DEVELOPER_DIR?.trim() ?? "");
  appendUniqueCandidate(candidates, seen, (await readSelectedXcodeDeveloperDirectory(env)) ?? "");
  for (const candidate of listApplicationXcodeDeveloperDirectories()) {
...
async function validateXcodeEnvironmentCandidate(
  candidate: string,
  env: HostToolingEnvironment,
...
export async function resolveXcodeEnvironment(
  ...
  for (const candidate of await listXcodeDeveloperDirectoryCandidates(env)) {
    const resolvedEnvironment = await validateXcodeEnvironmentCandidate(candidate, env);
...
export async function listIosSchemes(
  ...
  const listingResult = await runHostCommand(
    [
      xcodeEnvironment.xcodebuildBin,
      "-list",
...
export function ensureAndroidLocalProperties(repoRoot: string, sdkRoot: string): void {
...
export function formatProvisioningDiagnostics(stdout: string, stderr: string): string {
  const segments = [stdout.trim(), stderr.trim()].filter((segment) => segment.length > 0);
  return segments.join("\n");
}

/** Format a stable doctor status line. */
export function formatDoctorStatus(name: string, status: string): string {
  return `${name.padEnd(DOCTOR_STATUS_NAME_WIDTH, " ")} ${status}`;
}

/** Format a subprocess failure result for doctor/status output. */
export function formatProvisioningStatus(
  result:
    | HostProvisionFailure
