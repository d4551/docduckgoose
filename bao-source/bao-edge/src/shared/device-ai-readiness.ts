...
import { getHostToolingEnvironment, type HostToolingEnvironment } from "./host-tooling";
...
export interface DeviceAiReadinessEnvironment {
  ...
  readonly BAO_EDGE_IOS_BUILD_MODE?: string;
  /** Force device-AI protocol execution on local hosts. */
  readonly BAO_EDGE_VERIFY_DEVICE_AI_PROTOCOL?: string;
  /** CI host indicator. */
  readonly CI?: string;
...
export function createDeviceAiReadinessEnvironment(
  ...
  return {
    ...(typeof env.BAO_EDGE_IOS_BUILD_MODE === "string"
      ? { BAO_EDGE_IOS_BUILD_MODE: env.BAO_EDGE_IOS_BUILD_MODE }
      : {}),
    ...(typeof env.BAO_EDGE_VERIFY_DEVICE_AI_PROTOCOL === "string"
      ? { BAO_EDGE_VERIFY_DEVICE_AI_PROTOCOL: env.BAO_EDGE_VERIFY_DEVICE_AI_PROTOCOL }
      : {}),
    ...(typeof env.CI === "string" ? { CI: env.CI } : {}),
    ...(typeof env.HF_TOKEN === "string" ? { HF_TOKEN: env.HF_TOKEN } : {}),
    ...(typeof env.HUGGINGFACE_HUB_TOKEN === "string"
      ? { HUGGINGFACE_HUB_TOKEN: env.HUGGINGFACE_HUB_TOKEN }
      : {}),
    ...(typeof env.ANDROID_SDK_ROOT === "string" ? { ANDROID_SDK_ROOT: env.ANDROID_SDK_ROOT } : {}),
    ...(typeof env.ANDROID_HOME === "string" ? { ANDROID_HOME: env.ANDROID_HOME } : {}),
    ...(typeof env.DEVELOPER_DIR === "string" ? { DEVELOPER_DIR: env.DEVELOPER_DIR } : {}),
  };
}
...
export function iosBuildIsDelegated(env: DeviceAiReadinessEnvironment): boolean {
  return (env.BAO_EDGE_IOS_BUILD_MODE?.trim() || "native") === "delegate";
}
...
export function shouldRunDeviceAiProtocol(
  ...
  if ((env.BAO_EDGE_VERIFY_DEVICE_AI_PROTOCOL?.trim() || "") === "1") {
    return true;
  }
  ...
  const requirements: DeviceAiRequirementStatus[] = [
    {
      ...
      satisfied: true,
      failureMessage:
        "Hugging Face access is validated against the required model endpoint during protocol execution.",
    },
    {
      code: "android_adb",
