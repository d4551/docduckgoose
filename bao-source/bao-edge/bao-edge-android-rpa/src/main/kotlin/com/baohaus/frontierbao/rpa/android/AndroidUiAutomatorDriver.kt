...
import com.baohaus.baoedge.core.driver.DriverAdapter
import com.baohaus.baoedge.core.driver.DriverExecutionConfig
import com.baohaus.baoedge.core.error.DriverArtifacts
import com.baohaus.baoedge.core.error.DriverExecutionReport
import com.baohaus.baoedge.core.error.ErrorCategory
import com.baohaus.baoedge.core.error.ExecutionError
import com.baohaus.baoedge.core.error.ExecutionResultEnvelope
import com.baohaus.baoedge.core.error.StepReport
import com.baohaus.baoedge.core.error.StepStatus
import com.baohaus.baoedge.core.flow.CommandTarget
import com.baohaus.baoedge.core.flow.Direction
import com.baohaus.baoedge.core.flow.FlowCommand
import com.baohaus.baoedge.core.flow.FlowExecutionState
import com.baohaus.baoedge.core.flow.FlowV1
import java.io.File
...
private constructor(
  ...
  override suspend fun execute(
    ...
    val correlationId = "bao-edge-rpa-${nowMs()}"
  ...
  private suspend fun executeStep(
    ...
    when (step) {
      ...
      FlowCommand.ClipboardRead -> {
        ...
        val clip = clipboard.primaryClip ?: return DriverArtifacts(clipboardContent = "")
        val item = clip.getItemAt(0) ?: return DriverArtifacts(clipboardContent = "")
        val text = item.coerceToText(context)?.toString() ?: ""
        return DriverArtifacts(clipboardContent = text)
      }

      is FlowCommand.ClipboardWrite -> {
        val clipboard = context.getSystemService(Context.CLIPBOARD_SERVICE) as? android.content.ClipboardManager
          ?: throw NonRetryableCommandException("clipboard service unavailable")
        val clip = android.content.ClipData.newPlainText("bao-flow", step.value)
  ...
  private fun captureArtifacts(device: UiDevice, correlationId: String, stepIndex: Int): DriverArtifacts {
    val artifactDir = File(context.cacheDir, "bao-edge-rpa-artifacts").apply { mkdirs() }
...
private fun mergeArtifacts(base: DriverArtifacts, update: DriverArtifacts?): DriverArtifacts {
  ...
  return DriverArtifacts(
    ...
    clipboardContent = update.clipboardContent ?: base.clipboardContent,
  )
}

/** Shared selector strategy: resourceId -> text -> contentDescription -> partial matches -> coordinates. */
object SelectorPriorityResolver {
  ...
  fun orderedSelectors(target: CommandTarget): List<BySelector> {
    ...
    val textContains = target.textContains
    if (!textContains.isNullOrBlank()) {
      selectors += By.textContains(textContains)
    }
    val textStartsWith = target.textStartsWith
    if (!textStartsWith.isNullOrBlank()) {
      selectors += By.textStartsWith(textStartsWith)
    }
    val contentDescriptionContains = target.contentDescriptionContains
    if (!contentDescriptionContains.isNullOrBlank()) {
      selectors += By.descContains(contentDescriptionContains)
    }
    return selectors
  }
}
