...
public enum FlowYamlParser {
    ...
    public static func parse(_ yaml: String) throws -> FlowV1 {
        ...
        guard let resolvedAppId = appId, !resolvedAppId.isEmpty else {
            throw NSError(domain: "BaoEdge", code: 1, userInfo: [NSLocalizedDescriptionKey: "Missing appId"])
        }
    ...
    private static func parseStep(_ payload: String) throws -> FlowCommand {
        if !payload.contains(":") {
            ...
            if payload == "readVisibleState" { return .readVisibleState }
            if payload == "clipboardRead" { return .clipboardRead }
            throw NSError(domain: "BaoEdge", code: 2, userInfo: [NSLocalizedDescriptionKey: "Unsupported step: \(payload)"])
        }
        ...
        if key == "assertText" {
            ...
            guard let targetStr = chunks.first, !targetStr.isEmpty, chunks.count > 1 else {
                throw NSError(domain: "BaoEdge", code: 5, userInfo: [NSLocalizedDescriptionKey: "assertText requires target::value"])
            }
        ...
        if key == "selectOption" {
            let chunks = value.components(separatedBy: "::")
            guard let targetStr = chunks.first, !targetStr.isEmpty, chunks.count > 1 else {
                throw NSError(domain: "BaoEdge", code: 6, userInfo: [NSLocalizedDescriptionKey: "selectOption requires target::option"])
            }
        ...
        throw NSError(domain: "BaoEdge", code: 3, userInfo: [NSLocalizedDescriptionKey: "Unsupported command: \(key)"])
    }
    ...
    private static func parseDirection(_ value: String) throws -> Direction {
        ...
        throw NSError(domain: "BaoEdge", code: 4, userInfo: [NSLocalizedDescriptionKey: "Unsupported direction: \(value)"])
    }
