...
public struct ControlPlaneRuntimeConfig: Sendable {
  ...
  /// Override with `BAO_EDGE_CONTROL_PLANE_MODEL_STATE_ID_PREFIX` when isolated state is required.
  ...
  public init(
    ...
    self.baseUrl =
      baseUrl
      ?? Self.resolveString(
        key: "BAO_EDGE_CONTROL_PLANE_BASE_URL",
        defaultValue: "http://127.0.0.1:3310"
      )
    self.pollIntervalMs =
      pollIntervalMs
      ?? Self.resolveInt(
        key: "BAO_EDGE_CONTROL_PLANE_POLL_INTERVAL_MS",
        defaultValue: 900,
        minValue: 50
      )
    self.pollAttempts =
      pollAttempts
      ?? Self.resolveInt(
        key: "BAO_EDGE_CONTROL_PLANE_POLL_ATTEMPTS",
        defaultValue: 180,
        minValue: 1
      )
    self.defaultPullTimeoutMs =
      ...
      ?? Self.resolveInt(
        key: "BAO_EDGE_CONTROL_PLANE_DEFAULT_PULL_TIMEOUT_MS",
    ...
    let defaultModelSourceValue =
      (defaultModelSource
      ?? Self.resolveString(
        key: "BAO_EDGE_CONTROL_PLANE_DEFAULT_MODEL_SOURCE"
      ))
    ...
    self.requestTimeoutSeconds =
      ...
      ?? TimeInterval(
        Self.resolveInt(
          key: "BAO_EDGE_CONTROL_PLANE_REQUEST_TIMEOUT_SECONDS",
          defaultValue: 20,
          minValue: 1
        )
      )
    self.modelStateIdPrefix =
      modelStateIdPrefix
      ?? Self.resolveString(
        key: "BAO_EDGE_CONTROL_PLANE_MODEL_STATE_ID_PREFIX",
        defaultValue: "model-state"
      )
    self.deviceAiRequiredModelRef =
      deviceAiRequiredModelRef
      ?? Self.resolveString(
        key: "BAO_EDGE_REQUIRED_MODEL_REF",
    ...
    self.deviceAiRequiredModelRevision =
      ...
      ?? Self.resolveString(
        key: "BAO_EDGE_REQUIRED_MODEL_REVISION",
    ...
    self.deviceAiRequiredModelFileName =
      ...
      ?? Self.resolveString(
        key: "BAO_EDGE_REQUIRED_MODEL_FILE",
    ...
    self.deviceAiRequiredModelSha256 =
      ...
      ?? Self.resolveString(
        key: "BAO_EDGE_REQUIRED_MODEL_SHA256",
    ...
    self.deviceAiManagedModelDirectory =
      ...
      ?? Self.resolveString(
        key: "BAO_EDGE_DEVICE_AI_MODEL_DIRECTORY",
        defaultValue: "bao-edge-device-ai/models"
      )
    self.deviceAiManagedReportDirectory =
      deviceAiManagedReportDirectory
      ?? Self.resolveString(
        key: "BAO_EDGE_DEVICE_AI_REPORT_DIRECTORY",
        defaultValue: "bao-edge-device-ai/reports"
      )
    self.deviceAiProtocolTimeoutMs =
      deviceAiProtocolTimeoutMs
      ?? Self.resolveInt(
        key: "BAO_EDGE_DEVICE_AI_PROTOCOL_TIMEOUT_MS",
    ...
    self.deviceAiReportMaxAgeMinutes =
      ...
      ?? Self.resolveInt(
        key: "BAO_EDGE_DEVICE_AI_REPORT_MAX_AGE_MINUTES",
        defaultValue: 240,
        minValue: 1
      )
    self.deviceAiDownloadMaxAttempts =
      ...
      ?? Self.resolveInt(
        key: "BAO_EDGE_DEVICE_AI_DOWNLOAD_MAX_ATTEMPTS",
        defaultValue: 3,
        minValue: 1
      )
    self.deviceAiHfToken =
      (deviceAiHfToken
      ?? Self.resolveString(
        key: "BAO_EDGE_DEVICE_AI_HF_TOKEN",
