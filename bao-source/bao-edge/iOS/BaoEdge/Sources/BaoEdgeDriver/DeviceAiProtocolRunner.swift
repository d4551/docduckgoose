...
public final class DeviceAiProtocolRunner {
    ...
    private static let autorunEnvironmentKey = "BAO_EDGE_DEVICE_AI_PROTOCOL_AUTORUN"
    private static let modeEnvironmentKey = "BAO_EDGE_DEVICE_AI_PROTOCOL_MODE"
    private static let appIdEnvironmentKey = "BAO_EDGE_DEVICE_AI_PROTOCOL_APP_ID"
    private static let modelRefEnvironmentKey = "BAO_EDGE_DEVICE_AI_PROTOCOL_MODEL_REF"
    private static let revisionEnvironmentKey = "BAO_EDGE_DEVICE_AI_PROTOCOL_MODEL_REVISION"
    private static let fileNameEnvironmentKey = "BAO_EDGE_DEVICE_AI_PROTOCOL_MODEL_FILE"
    private static let sha256EnvironmentKey = "BAO_EDGE_DEVICE_AI_PROTOCOL_MODEL_SHA256"
    private static let tokenEnvironmentKey = "BAO_EDGE_DEVICE_AI_PROTOCOL_HF_TOKEN"
    private static let correlationIdEnvironmentKey = "BAO_EDGE_DEVICE_AI_PROTOCOL_CORRELATION_ID"
    private static let applicationSupportRootEnvironmentKey = "BAO_EDGE_DEVICE_AI_PROTOCOL_APPLICATION_SUPPORT_ROOT"
    private static let latestReportFileName = "latest.json"

    private let logger = Logger(subsystem: "com.baohaus.baoedge.driver", category: "DeviceAiProtocolRunner")
    ...
    public init(
        ...
        self.applicationSupportRootProvider = applicationSupportRootProvider ?? {
            if let explicitRoot = Self.applicationSupportRootOverrideURL(environment: resolvedEnvironment) {
                return Result {
                    if !fileManager.fileExists(atPath: explicitRoot.path) {
    ...
    public func run(request: DeviceAiProtocolRequest) async -> DeviceAiProtocolRunOutcome {
        ...
        let requestedReportFileName = "\(normalizedRequest.correlationId).json"
        logger.notice(
            "device_ai_protocol_run_started correlationId=\(normalizedRequest.correlationId, privacy: .public) appId=\(normalizedRequest.appId, privacy: .public) modelRef=\(normalizedRequest.modelRef, privacy: .public)"
        )
        persistInProgressReport(
            request: normalizedRequest,
            model: resolvedModel(for: normalizedRequest),
            message: "Native iOS device AI protocol bootstrap started.",
            stage: "ios-bootstrap",
            requestedReportFileName: requestedReportFileName
        )
        let reportResult = await buildReport(
            for: normalizedRequest,
            requestedReportFileName: requestedReportFileName
        )
        let outcome = persist(reportResult.report, requestedReportFileName: requestedReportFileName)
        logger.notice(
            "device_ai_protocol_run_completed correlationId=\(outcome.report.correlationId, privacy: .public) state=\(String(describing: outcome.report.state), privacy: .public) reportPath=\(outcome.latestReportURL.path, privacy: .public)"
        )
        return outcome
    }

    private func buildReport(
        for request: DeviceAiProtocolRequest,
        requestedReportFileName: String
    ) async -> (report: DeviceAiProtocolRunReport, artifactDirectory: URL) {
        ...
        persistInProgressReport(
            request: request,
            model: resolvedModel,
            message: "Preparing app-managed device AI storage at \(artifactDirectory.path).",
            stage: "ios-stage",
        ...
        guard let validationFailure = validationFailure(for: request) else {
            ...
            persistInProgressReport(
                request: request,
                model: resolvedModel,
                message: "Checking app-managed model artifact at \(destinationURL.path).",
                stage: "ios-stage",
                startedAt: downloadStartedAt,
                requestedReportFileName: requestedReportFileName
            )
            ...
            case .success(let outcome):
                ...
                persistInProgressReport(
                    ...
                    message: "Model artifact ready. Running the iOS device AI smoke flow.",
                    stage: "ios-smoke",
    ...
    private func persist(_ report: DeviceAiProtocolRunReport, requestedReportFileName: String) -> DeviceAiProtocolRunOutcome {
        ...
        if let data = dataResult.successValue {
            ...
            logger.notice(
                "device_ai_protocol_report_persisted correlationId=\(report.correlationId, privacy: .public) reportPath=\(reportURL.path, privacy: .public) latestPath=\(latestReportURL.path, privacy: .public)"
            )
    ...
    private func persistInProgressReport(
        request: DeviceAiProtocolRequest,
        model: DeviceAiProtocolResolvedModel?,
        artifact: DeviceAiProtocolArtifact? = nil,
        message: String,
        stage: String,
        ...
        let report = DeviceAiProtocolRunReport(
        ...
        _ = persist(report, requestedReportFileName: requestedReportFileName)
    }
    ...
    private static func applicationSupportRootOverrideURL(environment: [String: String]) -> URL? {
        guard let rawOverride = environment[applicationSupportRootEnvironmentKey]?.trimmed.nonEmpty else {
            return nil
        }
