...
public protocol HuggingFaceNetworking: Sendable {
  /// Execute one HTTP request and return a downloaded file URL + HTTP metadata.
...
public struct URLSessionHuggingFaceNetworking: HuggingFaceNetworking, @unchecked Sendable {
  ...
  public func download(for request: URLRequest) async throws -> (URL, HTTPURLResponse) {
    ...
    return (downloadedFileURL, httpResponse)
  }
}
...
public final class HuggingFaceModelManager: @unchecked Sendable {
  private let logger = Logger(subsystem: "com.baohaus.baoedge.driver", category: "HuggingFaceModelManager")
  ...
  public func downloadAndVerify(
    ...
    if let existingArtifact = try resolveExistingArtifact(
      request: request,
  ...
  private func resolveExistingArtifact(
    request: HuggingFaceModelDownloadRequest,
    destinationURL: URL
  ) throws -> HuggingFaceModelDownloadOutcome? {
    guard fileManager.fileExists(atPath: destinationURL.path) else {
      logger.debug(
        "hf_existing_artifact_missing correlationId=\(request.correlationId, privacy: .public) path=\(destinationURL.path, privacy: .public)"
      )
      return nil
    }
    let expectedDigest = request.expectedSha256?
      .trimmingCharacters(in: .whitespacesAndNewlines)
      .lowercased()
    guard let expectedDigest, !expectedDigest.isEmpty else {
      logger.notice(
        "hf_existing_artifact_checksum_skipped correlationId=\(request.correlationId, privacy: .public) path=\(destinationURL.path, privacy: .public)"
      )
      return nil
    }

    let actualDigest = try computeSha256(for: destinationURL)
    if actualDigest == expectedDigest {
      logger.notice(
        "hf_existing_artifact_reused correlationId=\(request.correlationId, privacy: .public) path=\(destinationURL.path, privacy: .public)"
      )
      let tempURL = destinationURL.appendingPathExtension("part")
    ...
    logger.error(
      "hf_existing_artifact_checksum_mismatch correlationId=\(request.correlationId, privacy: .public) path=\(destinationURL.path, privacy: .public)"
    )
  ...
  private func downloadAttempt(
    ...
    let (downloadedFileURL, response) = try await networking.download(for: urlRequest)
    defer {
      try? removeFileIfNeeded(downloadedFileURL)
    }
    try validateStatus(
      response.statusCode,
      correlationId: request.correlationId,
  ...
  private func moveDownloadedFile(_ sourceURL: URL, to destinationURL: URL, correlationId: String) throws {
    try removeFileIfNeeded(destinationURL)
    do {
      ...
      throw HuggingFaceModelManagerError(
        code: .ioFailure,
        message: "Failed moving the downloaded artifact into managed storage: \(error.localizedDescription)",
        retryable: false,
        correlationId: correlationId
      )
    }
  ...
  private func appendFileContents(from sourceURL: URL, to destinationURL: URL, correlationId: String) throws {
    if !fileManager.fileExists(atPath: destinationURL.path) {
      fileManager.createFile(atPath: destinationURL.path, contents: nil)
    }
    guard let sourceStream = InputStream(url: sourceURL) else {
      throw HuggingFaceModelManagerError(
        code: .ioFailure,
        message: "Unable to read the downloaded model fragment from disk.",
        retryable: false,
        correlationId: correlationId
      )
    }
    guard let handle = try? FileHandle(forWritingTo: destinationURL) else {
      throw HuggingFaceModelManagerError(
        code: .ioFailure,
        message: "Unable to open file for appending at \(destinationURL.path).",
        retryable: false,
        correlationId: correlationId
      )
    }
    ...
    while sourceStream.hasBytesAvailable {
      let count = sourceStream.read(&buffer, maxLength: bufferSize)
      if count < 0 {
        throw HuggingFaceModelManagerError(
          code: .ioFailure,
          message: "Failed while appending the downloaded model fragment.",
          retryable: false,
          correlationId: correlationId
        )
      }
