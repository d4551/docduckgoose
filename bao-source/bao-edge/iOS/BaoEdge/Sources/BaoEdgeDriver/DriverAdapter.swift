...
public final class DefaultDriverAdapter: DriverAdapter {
    ...
    public func execute(flow: FlowV1, correlationId: String) -> IosDriverReport {
        ...
        if flow.steps.allSatisfy({ $0 == .launchApp }) {
            return makeInAppLaunchSmokeReport(flow: flow, correlationId: correlationId)
        }

        let reason = "Full iOS automation execution requires an external XCTest host. The in-app driver only supports launchApp smoke validation."
        return IosDriverReport(
            completedSteps: 0,
    ...
    private func makeInAppLaunchSmokeReport(flow: FlowV1, correlationId: String) -> IosDriverReport {
        let formatter = ISO8601DateFormatter()
        let startedAt = formatter.string(from: Date())
        let endedAt = formatter.string(from: Date())
        let steps = flow.steps.enumerated().map { index, _ in
            IosDriverStepReport(
                commandIndex: index,
                commandType: "launchApp",
                state: .success,
                message: "In-app launch smoke passed for \(flow.appId).",
                startedAt: startedAt,
                endedAt: endedAt,
                durationMs: 0,
                error: nil,
