...
public final class IosXcTestDriver {
    private let logger = Logger(subsystem: "com.baohaus.baoedge.driver", category: "IosXcTestDriver")
...
private extension IosXcTestDriver {
    ...
    func executeCommand(
        ...
        case .screenshot:
            ...
            let filename = "bao-flow-ios-\(correlationId)-step-\(commandIndex).png"
        ...
        case .readVisibleState:
            guard let hierarchyPath = captureHierarchy(
                app: app,
                commandIndex: commandIndex,
                correlationId: correlationId,
                ...
                throw IosDriverError(
                    code: "IOS_VISIBLE_STATE_CAPTURE_FAILED",
                    category: "runtime",
                    commandIndex: commandIndex,
                    command: "readVisibleState",
                    reason: "Failed to capture the current iOS visible state.",
                    retryable: true,
                    correlationId: correlationId
                )
            }
            return IosDriverArtifact(
                artifactPath: hierarchyPath,
                contentType: "text/plain",
                createdAt: iso8601(Date()),
                correlationId: correlationId,
        ...
        case .clipboardRead:
            ...
            #endif
            let tempDir = screenshotDirectory.appendingPathComponent("bao-flow-clipboard-\(correlationId)-step-\(commandIndex).txt")
            try? content.write(to: tempDir, atomically: true, encoding: .utf8)
            return IosDriverArtifact(
                artifactPath: tempDir.path,
                contentType: "text/plain",
                createdAt: iso8601(Date()),
                correlationId: correlationId,
                hierarchyPath: nil
    ...
    func resolveElement(in app: XCUIApplication, target: CommandTarget) -> XCUIElement? {
        ...
        // Priority 4: textContains (partial label match).
        if let substring = target.textContains, !substring.isEmpty {
        ...
        // Priority 5: textStartsWith (prefix label match).
        if let prefix = target.textStartsWith, !prefix.isEmpty {
        ...
        // Priority 6: contentDescriptionContains (partial accessibility label match).
        if let substring = target.contentDescriptionContains, !substring.isEmpty {
            let predicate = NSPredicate(format: "label CONTAINS[c] %@", substring)
            let candidates: [XCUIElement] = [
                app.otherElements.matching(predicate).firstMatch,
                app.buttons.matching(predicate).firstMatch,
    ...
    func captureHierarchy(
        ...
        do {
            ...
            let hierarchyPath = screenshotDirectory
                .appendingPathComponent("bao-flow-ios-\(correlationId)-step-\(commandIndex)-hierarchy.txt")
...
private extension IosXcTestDriver {
    ...
    func commandType(_ command: FlowCommand) -> String {
        ...
        case .readVisibleState: return "readVisibleState"
