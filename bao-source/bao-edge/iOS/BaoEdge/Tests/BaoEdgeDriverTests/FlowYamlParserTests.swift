...
struct FlowYamlParserTests {
    ...
    func parsesBasicFlow() throws {
        ...
        #expect(flow.appId == "com.baohaus.baoedge.ios")
    ...
    func parsesCanonicalParityCommands() throws {
        ...
        if case .readVisibleState = flow.steps[10] {
            #expect(Bool(true))
        } else {
            Issue.record("Expected readVisibleState at step 11")
        }
    }
    ...
    @Test func toYamlProducesExpectedHeader() {
        let flow = FlowV1(appId: "com.baohaus.baoedge.ios", steps: [])
        let yaml = FlowYamlParser.toYaml(flow)
        #expect(yaml.hasPrefix("appId: com.baohaus.baoedge.ios"))
