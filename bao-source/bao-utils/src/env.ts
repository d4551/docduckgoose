...
const IS_BROWSER: boolean = hasGlobalObject("window") && hasGlobalObject("document");

type ImportMetaWithEnv = ImportMeta & { env?: Record<string, string> };
type RuntimeBun = { env?: object };
...
function isRuntimeBun(value: unknown): value is RuntimeBun {
  ...
  return env === undefined || (typeof env === "object" && env !== null);
}
...
function getServerEnv(key: string): string | undefined {
  ...
  if (!isRuntimeBun(bun) || !bun.env) {
    return;
  }
  const value = Reflect.get(bun.env, key);
