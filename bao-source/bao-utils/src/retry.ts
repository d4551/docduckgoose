...
export interface RetryOptions {
  ...
  useJitter?: boolean | undefined;
  /** Custom retry condition function */
  shouldRetry?: ((error: unknown, attempt: number) => boolean) | undefined;
  /** Callback invoked before each retry */
  onRetry?:
    | ((error: unknown, attempt: number, delay: number) => void | Promise<void> | undefined)
    | undefined;
...
export async function retryWithBackoff<T>(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    await sleep(decision.delayMs, signal === undefined ? undefined : { signal });
  }
