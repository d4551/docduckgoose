 ...
 *
 * @baohaus/bao-utils/drone-policy
 */
...
type VelocityVector = {
  ...
  vy?: number | null;
  vz?: number | null;
  yawRateDeg?: number | null | undefined;
};
...
function evaluateGeofencePolygonAltitude(
  ...
  polygon: DroneGeofenceConfig["polygons"][number],
  altitude: number,
): void {
...
function evaluateHorizontalVelocityLimit(
  violations: DronePolicyViolation[],
  maxHorizontalSpeedMps: number | undefined,
  vx: number | null,
  vy: number | null,
  context?: DronePolicyViolation["context"],
): void {
...
function evaluateScalarVelocityLimit(
  violations: DronePolicyViolation[],
  code: "VELOCITY_VERTICAL" | "YAW_RATE",
  label: "Vertical speed" | "Yaw rate",
  value: number | null,
  maxValue: number | undefined,
  field: string,
  context?: DronePolicyViolation["context"],
): void {
  if (typeof maxValue !== "number" || value === null || Math.abs(value) <= maxValue) {
    return;
  }
  pushViolation(
    violations,
    code,
    `${label} ${value.toFixed(2)} exceeds ${maxValue}`,
    "error",
    field,
    context,
  );
...
function evaluateGeofence(
  ...
  const containingPolygons = geofence.polygons.filter((polygon) =>
...
function evaluateVelocity(
  ...
  const { maxHorizontalSpeedMps, maxVerticalSpeedMps, maxYawRateDegS } = policy.limits;
  ...
  evaluateHorizontalVelocityLimit(violations, maxHorizontalSpeedMps, vx, vy, context);
  evaluateScalarVelocityLimit(
    violations,
    "VELOCITY_VERTICAL",
    "Vertical speed",
    vz,
