...
export interface BunExecOptions {
  ...
  onStdoutChunk?: BunProcessStreamChunkHandler | undefined;
  /** Optional stderr chunk callback for streaming consumers. */
  onStderrChunk?: BunProcessStreamChunkHandler | undefined;
}
...
function resolveSpawnRuntimeState(options: BunExecOptions): SpawnRuntimeState {
  ...
  return { hasTimeout, timeoutMs, timeoutSignal, signal };
}
  ...
  return [
    {
      ...
      ...(options.killSignal && timeoutMs === undefined ? { killSignal: options.killSignal } : {}),
    },
  ];
}
...
export function spawnBunCommand(options: BunExecOptions): SpawnBunCommandResult {
  const { hasTimeout, timeoutMs, timeoutSignal, signal } = resolveSpawnRuntimeState(options);
  ...
  const spawnArgs = buildSpawnArgs(options, stdin, stdout, stderr, signal, timeoutMs);

  const spawnResult = toResultSync(() => Bun.spawn([...options.cmd], ...spawnArgs));
  if (!spawnResult.ok) {
    return {
      ok: false,
      result: buildSpawnFailureResult(spawnResult.error, timeoutSignal, signal),
    };
  }
