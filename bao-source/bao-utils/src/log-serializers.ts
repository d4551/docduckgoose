...
function serializeErrorObject(error: Error, maxDepth: number): SerializedError {
  ...
  if (error.cause) {
    const cause = serializeError(error.cause, maxDepth - 1);
    if (cause !== undefined) {
...
function serializePlainErrorObject(error: object): SerializedError {
  ...
  return {
    type: typeof error.name === "string" ? error.name : UNKNOWN_ERROR_TYPE,
...
export function serializeError(error: unknown, maxDepth = 5): SerializedError | undefined {
  ...
  if (maxDepth <= 0) {
    return { type: "Error", message: TRUNCATED_CAUSE_MESSAGE };
  }

  if (error instanceof Error) {
    return serializeErrorObject(error, maxDepth);
  }

  // Handle non-Error objects with name/message properties
  if (typeof error === "object") {
    return serializePlainErrorObject(error);
  }

  // Primitive values (string, number, etc.)
  return { type: UNKNOWN_ERROR_TYPE, message: String(error) };
}
