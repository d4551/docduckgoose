...
import { DEFAULT_HTTP_RETRY_CONFIG } from "@baohaus/bao-constants/retries";
...
export interface FetchWithRetryOptions {
  ...
  operation?: string;
  /** Fetch implementation. Defaults to the runtime-native global fetch. */
  fetch?: NativeFetch;
}
...
export type NativeFetch = (input: RequestInfo | URL, init?: NativeFetchInit) => Promise<Response>;
...
async function executeFetchAttempt(
  ...
  const fetchInit: NativeFetchInit = {
    ...baseInit,
    ...(normalizedHeaders != null && { headers: normalizedHeaders }),
    ...(hasExplicitBody && init?.body !== undefined && { body: init.body }),
    ...(signal != null && { signal }),
  };
  const result = await toResultAsync(fetcher(fetchTarget, fetchInit));
  return result.ok ? { ok: true, value: result.value } : { ok: false, error: result.error };
}

type RetryAttemptOutcome = { done: true; response: Response } | { done: false };

function toRetryAbortError(error: unknown): Error {
  return error instanceof Error ? error : new Error(String(error));
}

async function resolveRetryResponseOutcome(
  response: Response,
  ctx: RetryContext,
  attempt: number,
  isFinalAttempt: boolean,
): Promise<RetryAttemptOutcome> {
  const shouldReturn = !(ctx.canRetry && ctx.retryOnStatus?.(response.status)) || isFinalAttempt;
  if (shouldReturn) {
...
async function resolveRetryErrorOutcome(
  error: unknown,
  ctx: RetryContext,
  attempt: number,
  isFinalAttempt: boolean,
  ...
  if (ctx.upstreamSignal?.aborted) {
    throw toRetryAbortError(error);
  }
  const shouldThrow = !(ctx.canRetry && isRetryableNetworkError(error)) || isFinalAttempt;
  if (shouldThrow) {
    throw toRetryAbortError(error);
  }
...
function resolveRetryAttemptOutcome(
  result: { ok: true; value: Response } | { ok: false; error: unknown },
  ctx: RetryContext,
  attempt: number,
  isFinalAttempt: boolean,
): Promise<RetryAttemptOutcome> {
  if (result.ok) {
    return resolveRetryResponseOutcome(result.value, ctx, attempt, isFinalAttempt);
  }
  return resolveRetryErrorOutcome(result.error, ctx, attempt, isFinalAttempt);
}
...
async function runSingleRetryAttempt(
  ...
  cleanup?.();
  return resolveRetryAttemptOutcome(result, ctx, attempt, isFinalAttempt);
}
