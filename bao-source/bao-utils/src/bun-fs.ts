...
import { toResultAsync } from "./async-result";
...
type ReaddirResult<TOptions extends ReaddirOptions | ReaddirOptionsWithFileTypes | undefined> =
  TOptions extends ReaddirOptionsWithFileTypes ? DirEntLike[] : string[];

function isReaddirOptionObject(
  options: ReaddirOptions | ReaddirOptionsWithFileTypes | undefined,
): options is ReaddirOptions | ReaddirOptionsWithFileTypes {
  return options !== undefined && typeof options === "object";
}

async function readNonRecursiveReaddir(
  dir: string,
  ...
  if (withFileTypes) {
    ...
    return onlyFiles ? entries.filter((entry) => entry.isFile()) : entries;
  }
  if (!onlyFiles) {
...
async function readRecursiveNames(dir: string, onlyFiles: boolean): Promise<string[]> {
...
async function readRecursiveDirents(dir: string, entries: string[]): Promise<DirEntLike[]> {
...
export async function readdir<
  TOptions extends ReaddirOptions | ReaddirOptionsWithFileTypes | undefined,
>(dir: string, options?: TOptions): Promise<ReaddirResult<TOptions>> {
  const withFileTypes = isReaddirOptionObject(options) && options.withFileTypes === true;
  const recursive = isReaddirOptionObject(options) && options.recursive === true;
  const onlyFiles = isReaddirOptionObject(options) && options.onlyFiles === true;

  if (!recursive) {
    return (await readNonRecursiveReaddir(
      dir,
      withFileTypes,
      onlyFiles,
    )) as ReaddirResult<TOptions>;
  }

  const entries = await readRecursiveNames(dir, onlyFiles);

  if (!withFileTypes) {
    return entries as ReaddirResult<TOptions>;
  }

  return (await readRecursiveDirents(dir, entries)) as ReaddirResult<TOptions>;
}
  ...
  const copyResult = await toResultAsync(
    nodeCp(src, dest, {
      ...
      ...(filter === undefined
        ? {}
        : {
            filter: async (source: string, destination: string) => filter(source, destination),
          }),
    }),
  );
  if (!copyResult.ok) {
