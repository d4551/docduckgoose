...
function pushSplitCommandToken(state: SplitCommandTokenState): void {
...
function consumeSplitCommandCharacter(state: SplitCommandTokenState, char: string): void {
  if (state.escaping) {
    state.current += char;
...
function splitCommandTokens(command: string): string[] {
  ...
  const state: SplitCommandTokenState = {
    tokens: [],
    current: "",
    inSingle: false,
    inDouble: false,
  ...
  for (const char of normalizedCommand) {
    consumeSplitCommandCharacter(state, char);
  }
