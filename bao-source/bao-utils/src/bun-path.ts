...
function appendRelativeSegments(state: ResolvedPathState, segment: string): void {
  for (const entry of segment.split("/")) {
    applySegment(state, entry);
  }
}

function applyResolvedCandidate(state: ResolvedPathState, segment: string): void {
  const candidate = parseResolvedCandidate(segment);
  state.isAbsolute = candidate.isAbsolute;
  state.drivePrefix = candidate.drivePrefix;
  state.out = [];

  for (const entry of candidate.segments) {
    applySegment(state, entry);
  }
}

function resolveWithCurrentWorkingDirectory(state: ResolvedPathState): string | null {
  ...
  for (const cwdSegment of cwdSegments.filter(Boolean)) {
    applySegment(state, cwdSegment);
  }
...
export function resolve(...segments: string[]): string {
  ...
  for (const segment of normalizedSegments) {
    ...
    if (!isAbsoluteNormalizedPath(segment)) {
      appendRelativeSegments(state, segment);
      continue;
    }
    applyResolvedCandidate(state, segment);
  }

  if (!state.isAbsolute) {
    const cwdResult = resolveWithCurrentWorkingDirectory(state);
    if (cwdResult !== null) {
      return cwdResult;
    }
  }
