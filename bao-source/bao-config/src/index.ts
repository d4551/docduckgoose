...
function applyResolvedPort(ports: RuntimePorts, key: keyof RuntimePorts, candidate: unknown): void {
  const resolvedPort = toFiniteNumber(candidate);
  if (resolvedPort !== null) {
    ports[key] = resolvedPort;
  }
}

function resolveRuntimePorts(rawPorts: RuntimeStateUpdate["ports"] | undefined): RuntimePorts {
  const ports: RuntimePorts = {};
  if (isMergeableRecord(rawPorts)) {
    for (const [key, value] of Object.entries(rawPorts)) {
      const resolvedPort = toFiniteNumber(value);
      if (resolvedPort !== null) {
        ports[key] = resolvedPort;
      }
    }
  ...
  applyResolvedPort(ports, "http", rawPorts?.http);
  applyResolvedPort(ports, "api", rawPorts?.api);
  applyResolvedPort(ports, "websocket", rawPorts?.websocket);
  return ports;
}

function resolveRuntimeEnvironment(initial: RuntimeStateUpdate | undefined): RuntimeEnvironment {
  ...
  if (resolvedNodeEnv !== undefined) {
    environment.nodeEnv = resolvedNodeEnv;
  }

  const resolvedMode = initial?.environment?.mode ?? getImportMetaEnv()?.MODE;
  if (resolvedMode !== undefined) {
    environment.mode = resolvedMode;
  }
  return environment;
}
...
export const createRuntimeState: (initial?: RuntimeStateUpdate) => RuntimeState = (
  ...
  ports: resolveRuntimePorts(initial?.ports),
  baseUrls: {
    origin: initial?.baseUrls?.origin ?? null,
    api: initial?.baseUrls?.api ?? (typeof window === "undefined" ? null : window.location.origin),
    apiBasePath: normalizeApiBasePath(initial?.baseUrls?.apiBasePath),
    websocket: initial?.baseUrls?.websocket ?? null,
  },
  environment: resolveRuntimeEnvironment(initial),
  database: {
