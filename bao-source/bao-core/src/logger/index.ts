...
import { hostname as osHostname } from "@baohaus/bao-utils/bun-os";
import { getEnvVar } from "@baohaus/bao-utils/env";
import { redactLogMetadata } from "@baohaus/bao-utils/log-redaction";
import { serializeError } from "@baohaus/bao-utils/log-serializers";
...
function isLogMetadataRecord(value: unknown): value is Record<string, unknown> {
...
function isLogLevel(value: string | undefined): value is LogLevel {
  return value !== undefined && LOG_LEVELS.some((level) => level === value);
}
...
export class SharedLogger {
  ...
  constructor(options: LoggerOptions = {}) {
    ...
    if (this.fileTransportEnabled) {
      ...
      const logDirectoryCandidate =
        options.logDirectory ?? (typeof logDirectoryRaw === "string" ? logDirectoryRaw : undefined);
      const logDirectory = logDirectoryCandidate ?? "./logs";

      if (logDirectory) {
        this.initializeFileTransport(logDirectory);
      }
    }
  }

  private initializeFileTransport(logDirectory: string): void {
    createFileWriters(logDirectory).then(
      (writers) => {
        this.fileWriters = writers;
        this.flushPendingFileEntries();
      },
      (error: unknown) => {
        const failure = this.formatEntry("error", "Failed to initialize file log transport", {
          error: serializeError(error),
          logDirectory,
        });
        this.writeToConsole(failure, [error]);
      },
    );
  }
  ...
  child(contextSuffix: string, overrides: LoggerOptions = {}): SharedLogger {
    ...
    if (overrides.logDirectory !== undefined) {
      childOptions.logDirectory = overrides.logDirectory;
    }
    return new SharedLogger(childOptions);
  }
  ...
  private emit(level: LogLevel, message: string, args: unknown[]): void {
    ...
    const metadata =
      serializedArgs.length === 1 && isLogMetadataRecord(serializedArgs[0])
        ? serializedArgs[0]
        : undefined;
