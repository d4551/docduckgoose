...
const BAO_TARGET_KIND_LIFECYCLE_POLICIES = {
  ...
  // Navigation contributions are pure descriptor projections — registering
  // a SidebarRegistration / NavRegistration mutates only in-memory tables
  // consumed by the SSR sidebar partial. No request handlers, no DB
  // migrations, no daemon restart.
  ...
  // Design system contributions are pure token / preset projections — they
  // mutate in-memory token maps consumed by the SSR style pipeline. No
  // request handlers, no DB migrations, no daemon restart.
  ...
  // Contribution surface registrations (settings-tab / palette / api-group /
  // tile-group) are descriptor-only — same lifecycle as `sidebar` / `nav`.
...
function resolveBaoManifestLifecyclePolicy(
  targets: readonly Pick<BaoInstallTargetBase, "kind">[],
