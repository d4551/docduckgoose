/**
 * BaoControlPlane setup-status shared policy constants.
 *
 * Centralizes cache key/channel and minimum clamp values used by
 * setup wizard BaoControlPlane status services to avoid literal drift.
 *
 * @shared/constants/bao-control-plane-status
 */

/**
 * Redis key prefix used for BaoControlPlane setup-status snapshot entries.
 */
export const BAO_CONTROL_PLANE_STATUS_CACHE_KEY_PREFIX: "bao-control-plane:status" =
  "bao-control-plane:status" as const;

/**
 * Redis lock-key suffix used for BaoControlPlane setup-status rebuild locks.
 */
export const BAO_CONTROL_PLANE_STATUS_LOCK_KEY_SUFFIX: "lock" = "lock" as const;

/**
 * Redis pub/sub invalidation channel used by BaoControlPlane setup-status snapshots.
 */
...
/**
 * Minimum in-memory cache TTL accepted by BaoControlPlane setup-status snapshots.
 */
export const BAO_CONTROL_PLANE_STATUS_MIN_CACHE_TTL_MS = 0;

/**
 * Minimum timeout accepted by BaoControlPlane setup-status network calls.
 */
export const BAO_CONTROL_PLANE_STATUS_MIN_TIMEOUT_MS = 1;

/**
 * Minimum Redis TTL (seconds) accepted by BaoControlPlane setup-status snapshots.
 */
