 ...
 * `@shared/schemas/bunbuddy.schemas.ts` because it aggregates multiple sources and decorates
...
import type { BunBuddyKind } from "../schemas/bunbuddy.schemas.ts";
import {
  BunBuddyCapabilitiesSnapshotsResponseSchema,
  BunBuddyCapabilitySnapshotSchema,
  BunBuddyDocsInfoSchema,
} from "../schemas/bunbuddy-capability-snapshot.schemas.ts";
...
export function isBunBuddyCapabilityNode(value: unknown): value is BunBuddyCapabilityNode {
  ...
  if (value.status !== "healthy" && value.status !== "degraded" && value.status !== "unreachable") {
    return false;
  }
  ...
  if (
    !(Array.isArray(value.endpoints) && value.endpoints.every((entry) => typeof entry === "string"))
  ) {
    return false;
  }
...
export function isBunBuddyCapabilityEdge(value: unknown): value is BunBuddyCapabilityEdge {
  ...
  if (
    !(
      ...
      value.requirements.every((entry) => typeof entry === "string")
    )
  ) {
    return false;
  }
