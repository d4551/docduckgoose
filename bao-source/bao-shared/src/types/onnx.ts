...
type GuardedJsonPrimitive = string | number | boolean | null;
type GuardedJsonValue = GuardedJsonPrimitive | GuardedJsonRecord | readonly GuardedJsonValue[];
type GuardedJsonRecord = { readonly [key: string]: GuardedJsonValue };

function isRecord(value: unknown): value is GuardedJsonRecord {
...
export function isOnnxModelsListResponse(value: unknown): value is OnnxModelsListResponse {
  ...
  if (!(Array.isArray(value.models) && value.models.every(isOnnxModelDto))) {
    return false;
  }
