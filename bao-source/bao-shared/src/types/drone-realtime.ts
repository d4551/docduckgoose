...
import type { WsJsonValue } from "./websocket/json";
 ...
 */
type DroneGuardJsonPrimitive = string | number | boolean | null;
type DroneGuardJsonValue =
  | DroneGuardJsonPrimitive
  | DroneGuardJsonRecord
  | readonly DroneGuardJsonValue[];
type DroneGuardJsonRecord = { readonly [key: string]: DroneGuardJsonValue };

function isDroneGuardRecord(value: unknown): value is DroneGuardJsonRecord {
  return value !== null && typeof value === "object" && !Array.isArray(value);
}

export function isDroneRealtimeMessage(value: unknown): value is DroneRealtimeMessage {
  if (!isDroneGuardRecord(value)) {
    return false;
  }
  const record = value;
