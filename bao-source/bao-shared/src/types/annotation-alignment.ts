...
} from "../schemas/annotation-alignment.schemas.ts";
import { isRecord } from "../utils/type-guards.ts";
import { AI_PROVIDER_KEYS, type AiProviderKeyResolved } from "./ai-providers.ts";
import { isCapabilityOwnershipFocusMap } from "./capability-ownership.guards-aggregates.ts";
import { isCapabilityOwnershipError } from "./capability-ownership.guards-foundation.ts";
...
export function isAnnotationAlignmentProvider(
  ...
  if (
    !(Array.isArray(value.libraries) && value.libraries.every((item) => typeof item === "string"))
  ) {
    return false;
  }
  return true;
