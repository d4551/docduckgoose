...
function buildDefaultValues<TSchema extends BunCliOptionSchema>(
  options: TSchema,
...
function handleArgumentToken<TSchema extends BunCliOptionSchema>(
  raw: string,
  args: readonly string[],
...
export function parseArgs<TSchema extends BunCliOptionSchema>(
  ...
  const state: ParseState<TSchema> = {
    values: buildDefaultValues(options),
    positionals: [],
    index: 0,
  };
  ...
  for (state.index = 0; state.index < args.length; state.index += 1) {
    ...
    if (handleArgumentToken(raw, args, state, options, byLong, byShort, allowPositionals)) {
      break;
    }
  }
