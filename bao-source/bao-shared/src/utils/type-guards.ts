...
import { STATUS } from "../constants/status-unified.js";
import type { Device, DeviceStatusType } from "../schemas/device.schemas.js";
...
export function isModelOfflineReady(model: unknown): model is AiModel & { offline_ready: true } {
  ...
  return Boolean(
    Reflect.get(model, "offline_ready") === true ||
      Reflect.get(model, "offline_ready") === 1 ||
      Reflect.get(model, "storage_path") ||
      Reflect.get(model, "storagePath"),
  );
}
...
export function isBrowserRuntime(): boolean {
  ...
  return !Object.hasOwn(window, "__TAURI__");
}
...
export function isRetryableError(error: unknown): boolean {
  ...
  if (status === HTTP_TOO_MANY || status === HTTP_UNAVAILABLE || status === HTTP_GATEWAY_TIMEOUT) {
    return true;
  }
...
export function isRouterRelatedError(error: unknown): boolean {
  ...
  return (
    ...
    (typeof message === "string" && (message.includes("navigation") || message.includes("route")))
  );
}
 ...
 *
 * Alias for `@shared/types/responses.isValidationError` to keep a stable import
