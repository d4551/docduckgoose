...
import { parseStrictBoolean } from "../config/env-boolean";
import {
...
export function resolveBaoControlPlaneContainerBuildProfile(params: {
  value?: string | null;
  env?: Record<string, string | undefined>;
  defaultValue?: BaoControlPlaneContainerBuildProfile;
}): BaoControlPlaneContainerBuildProfile {
...
export function resolveBaoControlPlanePlatformRuntime(
  ...
  const buildPlatforms =
    params.buildPlatforms && params.buildPlatforms.length > 0
      ? [...params.buildPlatforms]
      : params.preferHostBuildPlatformWhenUnset &&
          !normalizeOptionalString(env.BAO_CONTROL_PLANE_PLATFORMS) &&
          hostArch
        ? [toBaoControlPlaneLinuxPlatform(hostArch)]
        : resolveBaoControlPlaneBuildPlatforms(
  ...
  const containerBuildProfile = resolveBaoControlPlaneContainerBuildProfile({
    value: params.containerBuildProfile,
    env,
    defaultValue: params.defaultContainerBuildProfile,
  });
  const localDevImages =
    params.localDevImages ??
    parseOptionalStrictBoolean(env.BAO_CONTROL_PLANE_LOCAL_DEV_IMAGES) ??
    params.defaultLocalDevImages ??
    false;
  const registryCacheEnabled =
    params.registryCacheEnabled ??
    parseOptionalStrictBoolean(env.BAO_CONTROL_PLANE_IMAGE_REGISTRY_CACHE_ENABLED) ??
    params.defaultRegistryCacheEnabled ??
    false;
  ...
  return {
    ...
    bootstrapMode: resolveBaoControlPlaneBootstrapMode({
      requestedProvider,
      resolvedProvider,
      kubeContextReachable: params.kubeContextReachable,
    }),
