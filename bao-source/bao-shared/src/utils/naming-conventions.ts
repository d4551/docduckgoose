 ...
 *
 * @shared/utils/naming-conventions
 */
...
function collectNamingViolations(
  ...
  function visitVariableDeclaration(node: import("typescript").Node): void {
    ...
    const isFunctionInitializer = isFunctionLikeInit(tsModule, node.initializer);
    const baseType = isFunctionInitializer ? "function" : isConst ? "constant" : "variable";
    const resolvedType = resolveNameConventionType(baseType, name);
    recordViolation(resolvedType, name, node.name);
  }

  function visitFunctionDeclaration(node: import("typescript").Node): void {
  ...
  function visitClassDeclaration(node: import("typescript").Node): void {
