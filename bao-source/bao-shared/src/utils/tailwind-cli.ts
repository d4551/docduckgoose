...
const TAILWIND_NODE_MODULES_RELATIVE_PATH = "libs/happydumpling/node_modules";
const TAILWIND_CLI_ENTRY_RELATIVE_PATH = `${TAILWIND_NODE_MODULES_RELATIVE_PATH}/@tailwindcss/cli/dist/index.mjs`;
const TAILWIND_CLI_BIN_RELATIVE_PATH = `${TAILWIND_NODE_MODULES_RELATIVE_PATH}/.bin/tailwindcss`;
...
function buildTailwindCliCandidate(
  rootPath: string,
  entryRelativePath: string,
): ResolvedTailwindCliLocation {
  return {
    entryPath: path.join(rootPath, entryRelativePath),
    nodeModulesDir: path.join(rootPath, TAILWIND_NODE_MODULES_RELATIVE_PATH),
  };
}
...
function resolveTailwindCandidateRoots(): readonly string[] {
 ...
 */
function resolveTailwindCliCandidates(): readonly ResolvedTailwindCliLocation[] {
  return resolveTailwindCandidateRoots().flatMap((rootPath) => [
    buildTailwindCliCandidate(rootPath, TAILWIND_CLI_ENTRY_RELATIVE_PATH),
    buildTailwindCliCandidate(rootPath, TAILWIND_CLI_BIN_RELATIVE_PATH),
  ]);
}
 ...
 */
async function resolveTailwindCliLocation(): Promise<ResolvedTailwindCliLocation> {
  for (const candidate of resolveTailwindCliCandidates()) {
    if (await exists(candidate.entryPath)) {
      return candidate;
    }
  }

  const checkedPaths = resolveTailwindCliCandidates().map((candidate) => candidate.entryPath);
  throw new Error(
 ...
 */
async function resolveTailwindCliEntryPath(): Promise<string> {
  const resolvedLocation = await resolveTailwindCliLocation();
...
export async function buildTailwindCliCommand(
  ...
  const cliEntryPath = await resolveTailwindCliEntryPath();
  return [
