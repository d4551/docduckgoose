...
function estimateDurationMs(
  ...
  const durationMs =
    commandType === "joint"
      ? estimateJointDurationMs(target, current, maxVelocity)
      : commandType === "pose"
        ? estimatePoseDurationMs(target, current, maxVelocity)
        : sampleIntervalMs;

  return clamp(durationMs, sampleIntervalMs, maxDurationMs);
}
...
function estimateJointDurationMs(
  ...
  return (maxDelta / maxVelocity) * MS_PER_SECOND;
}
...
function estimatePoseDurationMs(
  ...
  return (distance / maxVelocity) * MS_PER_SECOND;
}
